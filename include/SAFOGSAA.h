...
private:
  ...
  public:
    ...
    void calculateScores(int newP1, int newP2, int M, int N, AlignType aType,
      ...
      const ScoreSystemType Mismatch =
          AllowMismatch ? scoreSystem.getMismatchPenalty()
                        : std::numeric_limits<ScoreSystemType>::min();
  ...
  class HashPriorityQueue {
    ...
    std::vector<std::priority_queue<Node, std::vector<Node>,
                                    std::greater<Node>>> *table;
