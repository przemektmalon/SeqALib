...
private:
  ...
  computeScoreVectors(ContainerType &Seq1, ContainerType &Seq2, float tbe) {
  ...
  template <typename ArrayType>
  void buildResultRec(ArrayType &Seq1, ArrayType &Seq2,
                      AlignedSequence<Ty, Blank> &Result, int M, int N, int tb,
                      int te) {
    ...
    if (N == 0) {
      if (M > 0) {
        for (auto Char : Seq1) {
          Data.push_back(typename BaseType::EntryType(Char, Blank, false));
        }
      }
    } else if (M == 0) {

      for (auto Char : Seq2) {
        Data.push_back(typename BaseType::EntryType(Blank, Char, false));
      }
    } else if (M == 1) {
      ...
      for (int j = 1; j < N + 1; j++) {
        ...
        if (AllowMismatch) {
          maxTemp = std::max(GapOpen + (GapExtend * (j - 1)) + Similarity +
                                 GapOpen + (GapExtend * (N - j)),
                             max);
        } else {
          if (isMatch) {
            maxTemp = std::max(GapOpen + (GapExtend * (j - 1)) + Similarity +
      ...
      for (int i = 1; i < N + 1; i++) {
        if (i == index) {
          ...
          if (!AllowMismatch && !isValidMatch) {
            ...
            Data.push_back(typename BaseType::EntryType(Seq1[0], Seq2[i - 1],
                                                        isValidMatch));
          }
        } else {
          Data.push_back(
      ...
      if (!type) { // If a type 1 midpoint
        ...
        buildResultRec(newSeq1Temp, newSeq2Temp, Result, M - mid, N - index, g,
                       te);
      } else { // If a type 2 midpoint
        // Recurse on the newfound indices
        ArrayType newSeq1(Seq1);
        ...
        Data.push_back(typename BaseType::EntryType(Seq1[mid], Blank, false));
        buildResultRec(newSeq1Temp, newSeq2Temp, Result, M - mid - 1, N - index,
                       0, te);
      }
    }
  }
...
public:
  ...
  virtual AlignedSequence<Ty, Blank> getAlignment(ContainerType &Seq1,
    ...
    buildResultRec(View1, View2, Result, Seq1.size(), Seq2.size(), GapOpen,
                   GapOpen);
    clearAll();
    return Result;
  }
